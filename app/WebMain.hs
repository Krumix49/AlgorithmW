...
import Derivation.Trace (inferPseudoFunctionSource, inferSource)
...
app = do
  ...
  S.post "/infer" $ do   -- 输入表达式后提交，使用post方法，向/infer路由发送POST请求，表单字段source的值作为source参数传递给inferSource函数。
    ...
    params <- S.formParams
    let mode = maybe "expr" T.unpack (lookup "mode" params)
    let outcome =
          case mode of
            "pseudo-function" -> inferPseudoFunctionSource source
            _ -> inferSource source
